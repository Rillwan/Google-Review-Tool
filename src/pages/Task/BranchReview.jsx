...
const BranchReview = ({ props }) => {
    ...
    const HandleReview = async ({ text }) => {
        ...
        window.open(`${props?.RLink}`, '_blank');
    }
                    ...
                    <div className='relative p-4 h-full'>
                        <div className='absolute top-1/2 left-1/2 -translate-1/2'>
                            <button onClick={() => HandleReview({ text: text[0]?.text })}
                                className='px-6 py-2 bg-green-700 text-white rounded-lg font-medium hover:opacity-[0.8]'>Go to Review - {props?.branch} Branch</button>
                        </div>
                    </div>
                </div>
            </div>
